(() => {
  ...
  road.innerHTML = `
    <div class="journey-topline">
      <span>LIVE CANDIDATE JOURNEY</span>
      <b>10 milestones · one connected process</b>
    </div>
    <svg class="journey-svg" viewBox="0 0 1200 560" aria-label="Candidate journey roadmap from profile to offer" role="img">
      <defs>
        <linearGradient id="journeyRoad" x1="0" x2="1">
          <stop offset="0" stop-color="#2d91ff"/>
          <stop offset=".42" stop-color="#2ad7aa"/>
          <stop offset=".72" stop-color="#ffb52e"/>
    ...
    <div class="journey-nodes journey-top"></div>
    <div class="journey-nodes journey-bottom"></div>
  ...
  const node = (s, index) => `
    <article class="journey-node node-${index + 1}" data-index="${index}">
      <div class="node-pin"><span>${s[0]}</span></div>
      <div class="node-card">
        <div class="node-label">${s[3]}</div>
        <h3>${s[1]}</h3>
        <p>${s[2]}</p>
      </div>
    </article>`;
  ...
  let active = 0;
  const activate = () => {
    all.forEach((n,i) => n.classList.toggle('active', i <= active));
    active = (active + 1) % all.length;
  };
  all.forEach((n,i) => n.addEventListener('mouseenter', () => {
    active = i;
    all.forEach((x,j) => x.classList.toggle('active', j <= i));
  }));
  all.forEach((n,i) => n.addEventListener('click', () => {
    active = i;
    all.forEach((x,j) => x.classList.toggle('active', j <= i));
  }));
